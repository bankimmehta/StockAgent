...
def compute_recommended_buy_rate(df, risk, strategy, analyst_target, current_p):
    ...
    risk_discount = {
        "Conservative (Safe)":       0.97,
        "Moderate (Balanced)":       0.99,
        "Aggressive (High Growth)":  1.005,
    }
...
def fetch_quarterly_financials(ticker_obj):
    """
    Safely extracts the last 8 quarters of Revenue, Operating Income, 
    and Net Income from the quarterly statement.
    """
    ...
    try:
        # Fallback chains for structural differences in yfinance attributes
        q_income = getattr(ticker_obj, 'quarterly_income_stmt', None)
        if q_income is None or q_income.empty:
            q_income = getattr(ticker_obj, 'quarterly_financials', None)

        if q_income is None or q_income.empty:
            return empty_df

        # Case-insensitive row matching
        index_lower = [str(x).lower().strip() for x in q_income.index]

        def get_row_by_keywords(keywords):
            for kw in keywords:
                for idx, row_name in enumerate(index_lower):
                    if kw in row_name:
                        return q_income.iloc[idx]
            return None

        rev_series = get_row_by_keywords(['total revenue', 'revenue', 'operating revenue', 'sales'])
        op_series  = get_row_by_keywords(['operating income', 'operating profit', 'operatingincome'])
        net_series = get_row_by_keywords(['net income', 'netincome', 'net income common stockholders', 'net income from continuing operations'])

        data_dict = {}
        target_columns = q_income.columns[:8]  # Capture up to 8 quarters

        for col in target_columns:
            date_str = col.strftime('%Y-%m-%d') if hasattr(col, 'strftime') else str(col)
            data_dict[date_str] = {
                'Revenue':          rev_series[col] if (rev_series is not None and col in rev_series.index) else None,
                'Operating Income': op_series[col]  if (op_series is not None  and col in op_series.index)  else None,
                'Net Income':       net_series[col] if (net_series is not None and col in net_series.index) else None
            }

        df_q = pd.DataFrame(data_dict).T
        # Clean potential float values
...
class StockAgentEngine:
    ...
    def run(self, ticker, market_type):
        ...
        prompt = f"""
You are a professional institutional equity analyst. You analyze structural data and construct clear interpretative conclusions.
...
if st.button("Run Analysis"):
    ...
    if not api_key:
        ...
        with st.spinner("Processing market data feeds and structural indicators..."):
            try:
                ...
                if df is None:
                    ...
                    q_df = fetch_quarterly_financials(ticker_obj)

                    # Strategy-adjusted buy targets
                    ...
                    if hist_trades:
                        ...
                        st.warning("⚠️ No ≥20% profit windows found in the past 1 year. "
                                   "Clean long entries matching targets are currently zero.")
                    ...
                    # ==========================================
                    st.markdown("## 📊 Quarterly Financial Performance (Last 8 Quarters)")
                    if not q_df.empty:
                        # Print Data Frame
                        q_formatted = q_df.copy()
                        for col in q_formatted.columns:
                            q_formatted[col] = q_formatted[col].apply(format_currency_val)
                        st.dataframe(q_formatted.T, use_container_width=True)

                        # Graphic Visualization
                        fig_q = go.Figure()
                        fig_q.add_trace(go.Bar(x=q_df.index, y=q_df['Revenue'], name='Revenue', marker_color='#4169E1'))
                        fig_q.add_trace(go.Bar(x=q_df.index, y=q_df['Operating Income'], name='Operating Income', marker_color='#20B2AA'))
                        fig_q.add_trace(go.Bar(x=q_df.index, y=q_df['Net Income'], name='Net Income', marker_color='#32CD32'))
                        
                        fig_q.update_layout(
                            barmode='group',
                            title=f"{ticker_input} — Quarterly Revenues & Net Returns",
                            plot_bgcolor='rgba(15,15,15,1)', paper_bgcolor='rgba(0,0,0,0)',
                            xaxis=dict(gridcolor='rgba(50,50,50,0.2)'),
                            yaxis=dict(gridcolor='rgba(50,50,50,0.2)'),
                        ...
                        st.info("Quarterly income statements are currently unavailable for this asset structure.")
                    ...
                    if inst_data is not None and isinstance(inst_data, pd.DataFrame):
                        ...
                        st.info("Institutional holder details are currently unavailable.")
                    ...
                    ratio_rows = {
                        "Metric": [
                            ...
                            "Debt / Equity", "Return on Equity (ROE)", "Return on Assets (ROA)", "Profit Margin",
                            "Market Cap"
                        ],
                        "Value": [
                            ...
                            f"{fund['roe']:.2%}"             if fund['roe']              else "N/A",
                            f"{fund['roa']:.2%}"             if fund['roa']              else "N/A",
                            f"{fund['profit_margin']:.2%}"   if fund['profit_margin']    else "N/A",
                    ...
                    if fund['analyst_target'] and fund['analyst_target'] > 0:
                        ...
                        if pct_to_target < 20:
                            st.error(
                                ...
                                f"{pct_to_target:.1f}% upside. A +20% gain target requires "
                                f"the asset to reach {buy_rec['sell_target']:.2f}."
                            )
                        else:
                            st.success(
                                f"✅ Analyst 12M consensus ({fund['analyst_target']:.2f}) implies "
                                f"{pct_to_target:.1f}% upside — exceeding the 20% valuation threshold."
                            )
